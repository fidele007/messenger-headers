/*
* This header is generated by classdump-dyld 1.0
* on Saturday, March 18, 2017 at 1:27:40 PM Central European Standard Time
* Operating System: Version 10.2 (Build 14C92)
* Image Source: /var/containers/Bundle/Application/982F7718-F913-4C3D-AECC-191FDA119619/Messenger.app/Messenger
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <FBSharedFramework/MNServiceControllable.h>
#import <FBSharedFramework/FBViewerContextClassProvidable.h>

@protocol FBCancelable;
@class ScoutAdapter, NSMutableArray, NSString;

@interface MNScoutService : NSObject <MNServiceControllable, FBViewerContextClassProvidable> {

	ScoutAdapter* _scoutAdapter;
	NSMutableArray* _sources;
	BOOL _scheduleTimer;
	id<FBCancelable> _continuation;

}

@property (readonly) unsigned long long hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
-(void)_queryLogSources;
-(id)initWithProviderMapData:(id)arg1 ;
-(void)handleIdle;
-(void)dealloc;
-(void)stop;
-(void)_stopTimer;
-(void)registerProvider:(id)arg1 ;
-(void)start:(id)arg1 ;
-(void)didBackground;
-(void)willForeground;
@end

