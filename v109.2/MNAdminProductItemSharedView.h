/*
* This header is generated by classdump-dyld 1.0
* on Saturday, March 18, 2017 at 1:27:36 PM Central European Standard Time
* Operating System: Version 10.2 (Build 14C92)
* Image Source: /var/containers/Bundle/Application/982F7718-F913-4C3D-AECC-191FDA119619/Messenger.app/Messenger
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Messenger/Messenger-Structs.h>
#import <UIKit/UIView.h>

@class FBNetworkImageView, UILabel;

@interface MNAdminProductItemSharedView : UIView {

	FBNetworkImageView* _productImageView;
	UILabel* _productNameLabel;
	UILabel* _priceLabel;

}
-(void)configureWithProductItemName:(id)arg1 productItemPrice:(id)arg2 productItemImageURL:(id)arg3 imageDownloader:(id)arg4 ;
-(id)initWithFrame:(CGRect)arg1 ;
-(void)layoutSubviews;
-(CGSize)sizeThatFits:(CGSize)arg1 ;
@end

