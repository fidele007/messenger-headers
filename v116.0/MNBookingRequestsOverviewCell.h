/*
* This header is generated by classdump-dyld 1.0
* on Monday, May 8, 2017 at 12:27:58 AM Central European Summer Time
* Operating System: Version 10.2 (Build 14C92)
* Image Source: /var/containers/Bundle/Application/0B364788-EFE9-4CC1-B4D1-033CC2D7E2B9/Messenger.app/Messenger
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <UIKit/UITableViewCell.h>

@protocol MNNativeComponentFlowBookingRequestFragment;
@class UILabel;

@interface MNBookingRequestsOverviewCell : UITableViewCell {

	id<MNNativeComponentFlowBookingRequestFragment> _bookingRequest;
	UILabel* _dateLabel;
	UILabel* _monthLabel;
	UILabel* _titleLabel;
	UILabel* _subTitleLabel;
	UILabel* _statusLabel;

}
-(void)configureWithBookingRequest:(id)arg1 cellType:(unsigned long long)arg2 ;
-(void)_configLabelsWithBookingRequest:(id)arg1 cellType:(unsigned long long)arg2 ;
-(void)layoutSubviews;
-(id)initWithStyle:(long long)arg1 reuseIdentifier:(id)arg2 ;
@end

