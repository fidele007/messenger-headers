/*
* This header is generated by classdump-dyld 1.0
* on Monday, May 8, 2017 at 12:27:51 AM Central European Summer Time
* Operating System: Version 10.2 (Build 14C92)
* Image Source: /var/containers/Bundle/Application/0B364788-EFE9-4CC1-B4D1-033CC2D7E2B9/Messenger.app/Messenger
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Messenger/Messenger-Structs.h>
#import <UIKit/UIView.h>
#import <Messenger/FBPaymentsViewModelConfigurable.h>

@class UIView, MNPaymentsPeerToPeerAttachmentViewModel, UIButton, NSString;

@interface FBPaymengGroupRequestAttachmentActionsView : UIView <FBPaymentsViewModelConfigurable> {

	UIView* _middleSeperator;
	MNPaymentsPeerToPeerAttachmentViewModel* _viewModel;
	UIButton* _firstActionButton;
	UIButton* _secondActionButton;

}

@property (nonatomic,copy) MNPaymentsPeerToPeerAttachmentViewModel * viewModel;              //@synthesize viewModel=_viewModel - In the implementation block
@property (nonatomic,readonly) UIButton * firstActionButton;                                 //@synthesize firstActionButton=_firstActionButton - In the implementation block
@property (nonatomic,readonly) UIButton * secondActionButton;                                //@synthesize secondActionButton=_secondActionButton - In the implementation block
@property (readonly) unsigned long long hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
+(CGSize)sizeWithViewModel:(id)arg1 constrainedSize:(CGSize)arg2 ;
-(void)_updateFirstActionButton;
-(void)_updateSecondActionButton;
-(UIButton *)firstActionButton;
-(UIButton *)secondActionButton;
-(id)initWithFrame:(CGRect)arg1 ;
-(void)layoutSubviews;
-(CGSize)sizeThatFits:(CGSize)arg1 ;
-(void)tintColorDidChange;
-(MNPaymentsPeerToPeerAttachmentViewModel *)viewModel;
-(void)setViewModel:(MNPaymentsPeerToPeerAttachmentViewModel *)arg1 ;
@end

