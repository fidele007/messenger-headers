/*
* This header is generated by classdump-dyld 1.0
* on Monday, May 8, 2017 at 12:27:49 AM Central European Summer Time
* Operating System: Version 10.2 (Build 14C92)
* Image Source: /var/containers/Bundle/Application/0B364788-EFE9-4CC1-B4D1-033CC2D7E2B9/Messenger.app/Messenger
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Messenger/MNSingleViewModelLoading.h>
#import <Messenger/MNViewModelUpdateMonitoring.h>
#import <Messenger/MNMontageRankedUserModelControllerListener.h>

@protocol OS_dispatch_queue, MNViewModelUpdating;
@class MNMontageRankedUserModelController, NSObject, NSString;

@interface MNInboxMontageRankedUsersDataViewModelLoader : NSObject <MNSingleViewModelLoading, MNViewModelUpdateMonitoring, MNMontageRankedUserModelControllerListener> {

	MNMontageRankedUserModelController* _montageRankedUserModelController;
	NSObject*<OS_dispatch_queue> _queue;
	id<MNViewModelUpdating> _updater;

}

@property (readonly) unsigned long long hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
@property (assign,nonatomic,__weak) id<MNViewModelUpdating> updater;              //@synthesize updater=_updater - In the implementation block
-(void)loadViewModelForRequest:(id)arg1 withRequestId:(unsigned long long)arg2 ;
-(void)_applyUpdate;
-(id)initWithMontageRankedUserController:(id)arg1 queue:(id)arg2 ;
-(void)montageRankedUserModelControllerUpdated:(id)arg1 ;
-(void)cancelRequest:(unsigned long long)arg1 ;
-(void)setUpdater:(id<MNViewModelUpdating>)arg1 ;
-(id<MNViewModelUpdating>)updater;
@end

