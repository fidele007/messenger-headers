/*
* This header is generated by classdump-dyld 1.0
* on Monday, May 8, 2017 at 12:27:48 AM Central European Summer Time
* Operating System: Version 10.2 (Build 14C92)
* Image Source: /var/containers/Bundle/Application/0B364788-EFE9-4CC1-B4D1-033CC2D7E2B9/Messenger.app/Messenger
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Messenger/FBPaymentsPeerToPeerFeatureGatingControllerListener.h>
#import <Messenger/FBPaymentsPeerToPeerSendFlowControllerDelegate.h>
#import <Messenger/FBPaymentsEventListener.h>
#import <FBSharedFramework/FBClassProvidable.h>
#import <Messenger/MNComposerExtension.h>

@protocol MNComposerExtensionEditDelegate, MNComposerExtensionSendDelegate, MNComposerExtensionDelegate;
@class NSString, UIColor, FBUserSession, MNComposerExtensionTrayPresentationStrategy, MNNavigationCoordinator, MNThreadNavigationCoordinator, FBMessengerPaymentsPlugin, FBMobileConfigContextManager, FBLazyCreator;

@interface MNComposerPaymentExtension : NSObject <FBPaymentsPeerToPeerFeatureGatingControllerListener, FBPaymentsPeerToPeerSendFlowControllerDelegate, FBPaymentsEventListener, FBClassProvidable, MNComposerExtension> {

	FBUserSession* _session;
	MNComposerExtensionTrayPresentationStrategy* _presentationStrategy;
	MNNavigationCoordinator* _navigationCoordinator;
	MNThreadNavigationCoordinator* _threadNavigationCoordinator;
	FBMessengerPaymentsPlugin* _paymentsPlugin;
	FBMobileConfigContextManager* _mobileConfigContextManager;
	FBLazyCreator* _composerIconCreator;
	id<MNComposerExtensionEditDelegate> _editDelegate;
	id<MNComposerExtensionSendDelegate> _sendDelegate;
	id<MNComposerExtensionDelegate> _delegate;
	NSString* _placeholderText;
	NSString* _searchPlaceholderText;
	UIColor* _tintColor;

}

@property (readonly) unsigned long long hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
@property (assign,nonatomic,__weak) id<MNComposerExtensionDelegate> delegate;                      //@synthesize delegate=_delegate - In the implementation block
@property (assign,nonatomic,__weak) id<MNComposerExtensionEditDelegate> editDelegate;              //@synthesize editDelegate=_editDelegate - In the implementation block
@property (assign,nonatomic,__weak) id<MNComposerExtensionSendDelegate> sendDelegate;              //@synthesize sendDelegate=_sendDelegate - In the implementation block
@property (nonatomic,copy,readonly) NSString * placeholderText;                                    //@synthesize placeholderText=_placeholderText - In the implementation block
@property (nonatomic,copy,readonly) NSString * searchPlaceholderText;                              //@synthesize searchPlaceholderText=_searchPlaceholderText - In the implementation block
@property (nonatomic,retain) UIColor * tintColor;                                                  //@synthesize tintColor=_tintColor - In the implementation block
-(BOOL)_shouldUseNameSendMoney;
-(id)_composerIcon;
-(void)_logGroupSendEvent:(id)arg1 withThreadId:(id)arg2 recipientIds:(id)arg3 ;
-(void)_modallyPresentSendMoneyViewControllerWithFlowId:(id)arg1 recipientIDs:(id)arg2 invitees:(id)arg3 groupThreadKey:(id)arg4 introductionViewType:(long long)arg5 ;
-(void)featureGatingDidUpdate;
-(void)peerToPeerSendFlowController:(id)arg1 didCancelCheckoutFlowWithFlowId:(id)arg2 error:(id)arg3 ;
-(void)peerToPeerSendFlowController:(id)arg1 didCompleteSendFlowWithFlowId:(id)arg2 checkoutViewController:(id)arg3 ;
-(void)peerToPeerSendFlowController:(id)arg1 willCheckoutWithFlowId:(id)arg2 ;
-(void)peerToPeerSendFlowController:(id)arg1 didCheckoutWithFlowId:(id)arg2 ;
-(void)peerToPeerSendFlowController:(id)arg1 didFailCheckoutWithFlowId:(id)arg2 ;
-(void)didTriggerEventWithName:(id)arg1 announcerIdentifier:(id)arg2 collectedData:(id)arg3 ;
-(void)setUpWithInitiator:(id)arg1 ;
-(id)extensionModule;
-(BOOL)shouldPromote;
-(unsigned long long)segmentedControlStrategy;
-(void)didSelect:(BOOL)arg1 withInitiator:(id)arg2 ;
-(BOOL)shouldPresent:(BOOL)arg1 ;
-(id)presentationStrategy;
-(long long)inputSource;
-(BOOL)isEligible;
-(id)activeSubExtension;
-(id)subExtensions;
-(void)didUpdateComposingContext:(BOOL)arg1 ;
-(NSString *)searchPlaceholderText;
-(id)initWithProviderMapData:(id)arg1 ;
-(id)extensionIdentifier;
-(void)setDelegate:(id<MNComposerExtensionDelegate>)arg1 ;
-(void)dealloc;
-(id<MNComposerExtensionDelegate>)delegate;
-(long long)category;
-(UIColor *)tintColor;
-(void)setTintColor:(UIColor *)arg1 ;
-(id)tabBarItem;
-(long long)soundName;
-(id<MNComposerExtensionEditDelegate>)editDelegate;
-(void)setEditDelegate:(id<MNComposerExtensionEditDelegate>)arg1 ;
-(NSString *)placeholderText;
-(void)tearDown;
-(id<MNComposerExtensionSendDelegate>)sendDelegate;
-(void)setSendDelegate:(id<MNComposerExtensionSendDelegate>)arg1 ;
-(BOOL)isThirdParty;
@end

