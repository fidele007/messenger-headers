/*
* This header is generated by classdump-dyld 1.0
* on Monday, May 8, 2017 at 12:27:58 AM Central European Summer Time
* Operating System: Version 10.2 (Build 14C92)
* Image Source: /var/containers/Bundle/Application/0B364788-EFE9-4CC1-B4D1-033CC2D7E2B9/Messenger.app/Messenger
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <Messenger/Messenger-Structs.h>
#import <FBSharedFramework/FBContainerViewController.h>
#import <Messenger/FBAnimatedTransitionRunnerDelegate.h>
#import <UIKit/UINavigationBarDelegate.h>
#import <Messenger/FBUINavigationContainerViewDelegate.h>
#import <libobjc.A.dylib/CAAnimationDelegate.h>
#import <Messenger/FBAnimatedTransitionDelegate.h>
#import <Messenger/FBExpandableNavigationBarStatusBarAppearanceDelegate.h>

@protocol UINavigationControllerDelegate;
@class UIViewController, FBAnimatedPushTransition, FBAnimatedLegacyTransition, FBAnimatedTransitionRunner, NSMutableArray, UIView, FBKVOController, NSMapTable, UIPanGestureRecognizer, FBUINavigationContainerView, NSArray, UINavigationBar, NSString;

@interface FBUINavigationController : FBContainerViewController <FBAnimatedTransitionRunnerDelegate, UINavigationBarDelegate, FBUINavigationContainerViewDelegate, CAAnimationDelegate, FBAnimatedTransitionDelegate, FBExpandableNavigationBarStatusBarAppearanceDelegate> {

	Class _navigationBarClass;
	Class _toolbarClass;
	UIViewController* _rootViewController;
	FBAnimatedPushTransition* _animatedTransition;
	FBAnimatedLegacyTransition* _animatedLegacyTransition;
	FBAnimatedTransitionRunner* _animatedTransitionRunner;
	NSMutableArray* _viewControllers;
	CGPoint _queuedDismissalVelocity;
	BOOL _transitionRunning;
	UIView* _legacyTransitionContainerView;
	FBKVOController* _KVOController;
	NSMapTable* _viewControllerToContentInsetMap;
	BOOL _swipeEnabled;
	BOOL _interruptibilityEnabled;
	BOOL _animatesSetViewControllers;
	BOOL _forceLayoutOnNavigationEvent;
	BOOL _enqueueForcedLayoutOnNavigationEvent;
	BOOL _scrollAwayNavigationEnabled;
	BOOL _treatsAllNavigationBarsAsTranslucent;
	id<UINavigationControllerDelegate> _delegate;
	UIPanGestureRecognizer* _interactivePopGestureRecognizer;
	NSMutableArray* _navigationBarItems;

}

@property (nonatomic,retain) NSMutableArray * navigationBarItems;                                     //@synthesize navigationBarItems=_navigationBarItems - In the implementation block
@property (nonatomic,readonly) FBUINavigationContainerView * view; 
@property (nonatomic,readonly) UIViewController * topViewController; 
@property (nonatomic,readonly) UIViewController * visibleViewController; 
@property (nonatomic,copy,readonly) NSArray * viewControllers; 
@property (nonatomic,readonly) UINavigationBar * navigationBar; 
@property (assign,getter=isNavigationBarHidden,nonatomic) BOOL navigationBarHidden; 
@property (assign,nonatomic,__weak) id<UINavigationControllerDelegate> delegate;                      //@synthesize delegate=_delegate - In the implementation block
@property (nonatomic,readonly) UIPanGestureRecognizer * interactivePopGestureRecognizer;              //@synthesize interactivePopGestureRecognizer=_interactivePopGestureRecognizer - In the implementation block
@property (assign,nonatomic) BOOL swipeEnabled;                                                       //@synthesize swipeEnabled=_swipeEnabled - In the implementation block
@property (assign,nonatomic) BOOL interruptibilityEnabled;                                            //@synthesize interruptibilityEnabled=_interruptibilityEnabled - In the implementation block
@property (assign,nonatomic) BOOL animatesSetViewControllers;                                         //@synthesize animatesSetViewControllers=_animatesSetViewControllers - In the implementation block
@property (assign,nonatomic) BOOL forceLayoutOnNavigationEvent;                                       //@synthesize forceLayoutOnNavigationEvent=_forceLayoutOnNavigationEvent - In the implementation block
@property (assign,nonatomic) BOOL enqueueForcedLayoutOnNavigationEvent;                               //@synthesize enqueueForcedLayoutOnNavigationEvent=_enqueueForcedLayoutOnNavigationEvent - In the implementation block
@property (assign,nonatomic) BOOL scrollAwayNavigationEnabled;                                        //@synthesize scrollAwayNavigationEnabled=_scrollAwayNavigationEnabled - In the implementation block
@property (assign,nonatomic) BOOL treatsAllNavigationBarsAsTranslucent;                               //@synthesize treatsAllNavigationBarsAsTranslucent=_treatsAllNavigationBarsAsTranslucent - In the implementation block
@property (nonatomic,readonly) BOOL treatsNavigationBarAsTranslucent; 
@property (readonly) unsigned long long hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
-(BOOL)treatsNavigationBarAsTranslucent;
-(BOOL)hasOngoingTransition;
-(void)setInterruptibilityEnabled:(BOOL)arg1 ;
-(void)setScrollAwayNavigationEnabled:(BOOL)arg1 ;
-(void)setForceLayoutOnNavigationEvent:(BOOL)arg1 ;
-(void)setTreatsAllNavigationBarsAsTranslucent:(BOOL)arg1 ;
-(void)pushNavigationItemForViewController:(id)arg1 animated:(BOOL)arg2 ;
-(NSMutableArray *)navigationBarItems;
-(void)popNavigationItemForViewController:(id)arg1 animated:(BOOL)arg2 ;
-(CGRect)destinationFrameForViewController:(id)arg1 navigationBarHidden:(BOOL)arg2 ;
-(void)prepareNavigationBarForTransitionOfType:(unsigned long long)arg1 transitionContext:(id)arg2 isInteractive:(BOOL)arg3 ;
-(void)setViewControllers:(id)arg1 withDirection:(unsigned long long)arg2 animated:(BOOL)arg3 ;
-(void)animatedTransition:(id)arg1 willBeginWithTransitionType:(unsigned long long)arg2 transitionContext:(id)arg3 ;
-(void)animatedTransition:(id)arg1 willBeginInteractivelyWithTransitionType:(unsigned long long)arg2 transitionContext:(id)arg3 ;
-(void)animatedTransition:(id)arg1 didEndWithTransitionType:(unsigned long long)arg2 transitionContext:(id)arg3 completedSuccessfully:(BOOL)arg4 ;
-(id)fbui_tabBarContainerViewController;
-(id)fbui_defaultNavigationBarConfigForViewController:(id)arg1 ;
-(void)setSwipeEnabled:(BOOL)arg1 ;
-(BOOL)swipeEnabled;
-(void)expandableNavigationBarDidUpdatePreferredStatusBarVisibility:(id)arg1 ;
-(id)transitionContextForTransitionType:(unsigned long long)arg1 ;
-(void)animatedTransition:(id)arg1 didEndInteractivelyWithTransitionType:(unsigned long long)arg2 transitionContext:(id)arg3 shouldCommit:(BOOL)arg4 ;
-(BOOL)animatedTransition:(id)arg1 shouldBeginForTransitionType:(unsigned long long)arg2 point:(CGPoint)arg3 ;
-(void)fbui_setTabBarContainerViewController:(id)arg1 ;
-(void)_setViewControllers:(id)arg1 withDirection:(unsigned long long)arg2 animated:(BOOL)arg3 ;
-(void)_announceWillShowViewController:(id)arg1 fromViewController:(id)arg2 animated:(BOOL)arg3 ;
-(void)_announceDidShowViewController:(id)arg1 fromViewController:(id)arg2 animated:(BOOL)arg3 ;
-(void)_navigationBarAppearanceDidChange;
-(id)_viewControllerUnderneathViewController:(id)arg1 ;
-(void)_beginAppearanceTransitionForViewController:(id)arg1 fromViewController:(id)arg2 animated:(BOOL)arg3 ;
-(void)_endAppearanceTransitionForViewController:(id)arg1 fromViewController:(id)arg2 animated:(BOOL)arg3 ;
-(void)_updateFrameForViewController:(id)arg1 newFrame:(CGRect)arg2 ;
-(void)_adjustScrollViewInsetsForViewController:(id)arg1 ;
-(UIEdgeInsets)_scrollViewInsetsForViewController:(id)arg1 ;
-(double)fbui_topLayoutGuideLength;
-(id)transitionContextForTransitionType:(unsigned long long)arg1 animated:(BOOL)arg2 ;
-(void)_updateInterruptibilityState;
-(void)_cleanupAfterPresentationOfViewController:(id)arg1 fromViewController:(id)arg2 ;
-(void)_cleanupAfterDismissalOfViewController:(id)arg1 toViewController:(id)arg2 ;
-(void)_resolveFramesForTransitionContext:(id)arg1 ;
-(void)_cleanupAfterLegacyTransition:(id)arg1 ;
-(void)_addViewUnderneathViewController:(id)arg1 ;
-(void)_pushViewController:(id)arg1 animated:(BOOL)arg2 withVelocity:(CGPoint)arg3 ;
-(void)_prepareViewControllerForPresentation:(id)arg1 fromViewController:(id)arg2 animated:(BOOL)arg3 ;
-(id)_popViewControllerAnimated:(BOOL)arg1 withVelocity:(CGPoint)arg2 ;
-(void)_prepareViewControllerForDismissal:(id)arg1 toViewController:(id)arg2 animated:(BOOL)arg3 ;
-(void)_recursivelyRemoveAnimations:(id)arg1 positionOnly:(BOOL)arg2 ;
-(void)_updateNavigationBarInsets;
-(id)_viewControllerAtIndex:(unsigned long long)arg1 ;
-(void)_popViewControllerWithoutAnimationSkippingAppearanceMethods:(id)arg1 ;
-(void)_pushViewControllerWithoutAnimationSkippingAppearanceMethods:(id)arg1 ;
-(BOOL)animatesSetViewControllers;
-(void)navigationContainerView:(id)arg1 navigationBarAppearanceChanged:(BOOL)arg2 animated:(BOOL)arg3 ;
-(void)expandableNavigationBar:(id)arg1 didUpdateStatusBarVisibilityPercentage:(double)arg2 ;
-(void)_prepareRootViewController:(id)arg1 ;
-(void)_removeViewUnderneathViewController:(id)arg1 ;
-(BOOL)interruptibilityEnabled;
-(void)setAnimatesSetViewControllers:(BOOL)arg1 ;
-(BOOL)forceLayoutOnNavigationEvent;
-(BOOL)enqueueForcedLayoutOnNavigationEvent;
-(void)setEnqueueForcedLayoutOnNavigationEvent:(BOOL)arg1 ;
-(BOOL)scrollAwayNavigationEnabled;
-(BOOL)treatsAllNavigationBarsAsTranslucent;
-(void)setNavigationBarItems:(NSMutableArray *)arg1 ;
-(id)analyticsModule;
-(void)fb_setTopLayoutGuideLength:(double)arg1 ;
-(id)analyticsPresentedViewController;
-(void)setDelegate:(id<UINavigationControllerDelegate>)arg1 ;
-(void)dealloc;
-(BOOL)isKindOfClass:(Class)arg1 ;
-(id<UINavigationControllerDelegate>)delegate;
-(UINavigationBar *)navigationBar;
-(BOOL)isNavigationBarHidden;
-(UIViewController *)topViewController;
-(id)popViewControllerAnimated:(BOOL)arg1 ;
-(unsigned long long)supportedInterfaceOrientations;
-(BOOL)becomeFirstResponder;
-(BOOL)shouldAutorotate;
-(void)loadView;
-(void)viewWillAppear:(BOOL)arg1 ;
-(void)viewDidAppear:(BOOL)arg1 ;
-(void)viewWillLayoutSubviews;
-(id)methodSignatureForSelector:(SEL)arg1 ;
-(BOOL)navigationBar:(id)arg1 shouldPopItem:(id)arg2 ;
-(long long)positionForBar:(id)arg1 ;
-(void)viewWillTransitionToSize:(CGSize)arg1 withTransitionCoordinator:(id)arg2 ;
-(long long)preferredInterfaceOrientationForPresentation;
-(void)viewDidLoad;
-(BOOL)shouldAutomaticallyForwardRotationMethods;
-(BOOL)shouldAutomaticallyForwardAppearanceMethods;
-(void)pushViewController:(id)arg1 animated:(BOOL)arg2 ;
-(void)willRotateToInterfaceOrientation:(long long)arg1 duration:(double)arg2 ;
-(void)willAnimateRotationToInterfaceOrientation:(long long)arg1 duration:(double)arg2 ;
-(void)didRotateFromInterfaceOrientation:(long long)arg1 ;
-(id)childViewControllerForStatusBarStyle;
-(id)childViewControllerForStatusBarHidden;
-(UIViewController *)visibleViewController;
-(void)setViewControllers:(id)arg1 animated:(BOOL)arg2 ;
-(UIPanGestureRecognizer *)interactivePopGestureRecognizer;
-(void)setNavigationBarHidden:(BOOL)arg1 animated:(BOOL)arg2 ;
-(NSArray *)viewControllers;
-(id)popToViewController:(id)arg1 animated:(BOOL)arg2 ;
-(id)initWithRootViewController:(id)arg1 ;
-(id)initWithNavigationBarClass:(Class)arg1 toolbarClass:(Class)arg2 ;
-(void)setViewControllers:(NSArray *)arg1 ;
-(void)setNavigationBarHidden:(BOOL)arg1 ;
-(id)popToRootViewControllerAnimated:(BOOL)arg1 ;
-(void)forwardInvocation:(id)arg1 ;
-(void)_statusBarFrameDidChange:(id)arg1 ;
@end

